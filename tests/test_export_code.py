...
from .util import code_equal, create_nb
...
def test_writes_code():
    ...
    assert code_equal(actual, expected)


def test_writes_code_without_metadata():
    cfg = ImplectusConfiguration(source_dir=".", code_dir="package")
    nb = create_nb([nb_cell("code", "pass", dict(tags=["export"]))])
    actual = writes_code(nb, "main.py", cfg)
